...
6. 修正 harness 的一个覆盖缺口：端点未配置的构建上，依赖「通过界面产生会话内容」的
   6 个用例此前会连锁失败，现已改为显式 SKIP 并说明原因；
   桌面布局用例也改为「有消息时断言正文宽度/滚动区，无消息时仍验证外壳布局」。
...
| `npm run check:ui`（dev 5173） | ✅ 28/28 通过 |
| `npm run check:ui`（生产构建 + 类 Pages 4180） | ✅ 28/28 通过 |
| `npm run check:ui`（生产构建，端点未配置） | ✅ 15 通过 + 13 项按预期 SKIP（不误报失败） |
| `cd worker && npm run check` | ✅ 38/38 通过 |
| `npm run check:sse` | ✅ 30/30 通过 |
| 安全扫描（源码 + 产物） | ✅ 无 `sk-` 形态密钥、无 sourcemap |
| git 工作树 | ✅ 干净，80 个跟踪文件，无临时验证文件 |
