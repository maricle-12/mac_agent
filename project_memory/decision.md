...
## 架构名必须归一化后再比较（Mach-O 叫 x86_64，Node 叫 x64）

Decision: 在 `macho.mjs` 里建立架构族映射 `ARCH_FAMILIES`（`arm64: [arm64, aarch64, arm64e]`、
`x64: [x64, x86_64, amd64, x86-64]`）与 `archFamily()` / `sameArch()`；
`readMachO()` **同时**返回两套信息：`arch` / `arches`（Mach-O 头部原始名，用于显示与排查）
与 `archFamily` / `archFamilies`（归一化族，**用于所有断言与判断**）。
所有架构比较一律走归一化，禁止拿原始名和 `x64` / `arm64` 直接比。

Reason: CI 真实失败于 `x64 二进制架构不符：实际 x86_64`。
同一架构在两套命名体系里写法不同（Node 分发包/`--arch`/`process.arch` 用 `x64`，
Mach-O 头部的 cputype 是 `x86_64` 或 `arm64`）。**`arm64` 恰好同名**，所以 arm64 先通过、
只有 x64 暴露 —— 这种「只有一半会炸」的 bug 特别容易被漏掉。
更麻烦的是同一个错误还有两个**不报错**的变体：通用包判定 `macho.arches.includes('x64')` 永远为假
（于是静默降级成分架构 DMG，只留一句容易被忽略的日志）、静态断言同样会误报失败。

Impact: ① 凡是有「同一概念多种写法」的地方（架构名、平台名、路径分隔符），
都要建立映射并用归一化函数比较，不要依赖「两套写法碰巧一致」；
② 解析结果要**同时**提供原始值与归一化值，原始值用于显示/排查，归一化值用于判断 ——
既不失真，也不容易比错；③ 映射关系本身要有断言（`check:mac-assets` 覆盖 `x86_64 ↔ x64` 这一对
与合成 thin/fat Mach-O 的家族归属），并断言构建脚本不再跨体系直接比较（`check:paths`）；
④ 修这类 bug 要用**真实产物**验证：本次直接下载官方 `darwin-arm64` / `darwin-x64` 二进制
（校验官方 SHA-256）后断言，确认 `x86_64` 现在被判为 `x64`。

## 构建失败必须「自证原因」，不能只留一句「自检未通过」

Decision: 所有自检的失败输出都要包含足够的定位信息：
① 失败项的**名称与细节**必须出现在最终的错误信息里（不只是打印在中间某一行）；
② **只要是「有断言没过」就要打印诊断**，不能只在抛异常时打印 —— 包括被验程序自己的
`app.log` 末尾、以及每个实例实际使用的数据目录；
③ 涉及二进制的断言要打印**工具原文**（例如 `lipo -info`）与**逐文件清单**
（例如包内每个 Mach-O 的路径、原始架构名、架构族、是否满足目标），并明确指出哪个文件不合格。

Reason: CI 上真实出现过 `[失败] universal 的发行包自检未通过` 这一条 —— 前 20 多项都通过了，
但日志末尾只看得到这一句话，看不出是**哪一项**没过、也看不出与架构有没有关系，
只能再跑一轮去猜。远程构建（尤其 macOS）无法本地复现，日志就是唯一线索；
「失败信息不足」会让每一轮排查都多花一次 CI 往返。

Impact: 新增任何断言时，同时问两件事：
「失败时这行日志能不能定位到具体哪一项？」「能不能判断出是环境问题还是产物问题？」
凡是有外部工具能给出权威判定的（`lipo -info`、`codesign -dv`、`plutil -lint`、`hdiutil verify`），
都要把它的原文打进日志，而不是只留我们自己的解析结论。
这条与「环境依赖的验收项要显式跳过」是同一原则的两面：**失败要说清，跳过也要说清**。
