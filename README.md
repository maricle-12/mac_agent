...
| `npm run check:paths` | **跨平台路径自检**（50 项，含把 macOS / Linux 的目录规则在本机算一遍并断言、构建期路径与架构名不变量） |
| `npm run check:mac-assets` | **macOS 打包资源自检**（37 项：ICNS、Info.plist、Mach-O 解析器与架构名映射，可在任意平台跑） |
